...
$Global:self = @{
    ...
    Remediation = '<p>It is recommended to adjust the password replication policy on the RODC to deny replication of privileged user credentials to an RODC.</p><br /><p>You can review the current settings by opening <b>Active Directory Users and Computers</b>, locate the identified <b>RODC</b>, and then right-click and select <b>Properties</b>.</p><br /><p>In the <b>Properties</b> of the RODC, select the <b>Password Replication Policy</b> tab.</p><br /><p>Review the replication policies, identify the policy that is permitting replication of credentials, and adjust to ensure there is a deny policy for the privileged users.</p><br /><p>You may also need to review the membership of the group <b>Denied RODC Password Replication Group</b>, which by default holds many privileged user groups that should be denied replication.</p><h3>References</h3><p><a href="https://learn.microsoft.com/en-us/previous-versions/windows/it-pro/windows-server-2008-r2-and-2008/cc753223(v=ws.10)" target="_blank">RODC Features | Microsoft Learn</a></p>'
    Types = @('IoE')
    DataSources = @('AD.LDAP')
    OutputFields = @(
            ...
            foreach ($principalDN in $allowedIDs) {
                try {
                    ...
                    if ($principalObject.Attributes."objectclass"[-1] -eq "group") {
                        ...
                        $members = @()
                        $members += Search-AD -dnsDomain $domain -baseDN $DN -scope "Subtree" -filter "(&(memberOf:1.2.840.113556.1.4.1941:=$principalDN)(admincount=1)(objectClass=user))"

                        foreach ($member in $members) {
                            if ($outputObjects.DistinguishedName -notcontains $member.DistinguishedName) {
                                Get-CacheResult -targetObject $member
                            }
                        }
                    }
                    else {
                        if ($principalObject.Attributes."admincount"[0] -eq "1") {
                            if ($outputObjects.DistinguishedName -notcontains $principalObject.DistinguishedName) {
                                Get-CacheResult -targetObject $principalObject
                            }
                        }
                    }
                }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBs/m8hWlwhKg4F
# lVbFIC7VA9SLgH+O1+fGBOh+3+Oj4KCCE3wwggVyMIIDWqADAgECAhB2U/6sdUZI
...
# FTAvBgkqhkiG9w0BCQQxIgQggsFFYO8fVgiY9YufeX5Tt2uwQNCYSyU9us8NtwR7
# LCswDQYJKoZIhvcNAQEBBQAEggIAOIbUF5t5aSDBHJP0MuasVhJDLZx9/ebBPc4f
# 46RA4v+HnRmSpm5mAiADQr287SbBKKWSWBYQ7yFizxzpzWwE/B6Fl09WLStbHJWN
# krkED4fpIou79bc0HIBFmZ4KUpTHy2SIc94FNyIY3ND01Yj/dAeIM044kX/jaax8
# MbfDXfTQxSdrXUEvwt7bD0yJzNWtg/ZRdx7VjLEo7E3dcxzz6MomGaOOFD8fPQjV
# oZK1J1C9n/Y41XLT41esz1r5q7glFnR/U3wi3tVx1wkttRBab7pGHlSgiooOlv0f
# wwgbkYsHQM8MKb+0d2aqzrqE9lCRKb5BMTpRcn7VXrEUk4rEqvBb9RRsd8HI7ynT
